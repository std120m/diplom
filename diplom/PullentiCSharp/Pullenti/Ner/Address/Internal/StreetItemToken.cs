    ...
    public class StreetItemToken : Pullenti.Ner.MetaToken
    {
        ...
        static List<StreetItemToken> _tryParseList(Pullenti.Ner.Token t, int maxCount, Pullenti.Ner.Geo.Internal.GeoAnalyzerData ad)
        {
            ...
            for (; t != null; t = (t == null ? null : t.Next)) 
            {
                ...
                if (t.IsHiphen && sit != null && ((sit.Typ == StreetItemType.Name || sit.Typ == StreetItemType.StdName || ((sit.Typ == StreetItemType.StdAdjective && !sit.IsAbridge))))) 
                {
            ...
            for (int i = 0; i < (res.Count - 1); i++) 
            {
                if (res[i].Typ == StreetItemType.Name && res[i + 1].Typ == StreetItemType.Name && (res[i].WhitespacesAfterCount < 3)) 
                {
                    ...
                    {
                        if (isPers && res[i + 1].EndToken.GetMorphClassInDictionary().IsProperName && !res[i].EndToken.GetMorphClassInDictionary().IsProperName) 
                    ...
                    if (r.AltValue == null && res[i].BeginToken.GetMorphClassInDictionary().IsProperName) 
                        r.AltValue = Pullenti.Ner.Core.MiscHelper.GetTextValueOfMetaToken(res[i + 1], Pullenti.Ner.Core.GetTextAttr.No);
                    else if (r.AltValue == null && res[i + 1].BeginToken.GetMorphClassInDictionary().IsProperName) 
                        r.AltValue = Pullenti.Ner.Core.MiscHelper.GetTextValueOfMetaToken(res[i], Pullenti.Ner.Core.GetTextAttr.No);
            ...
            {
                if (!Pullenti.Ner.Geo.Internal.MiscLocationHelper.IsUserParamAddress(res[0])) 
                {
        ...
        public static void Initialize()
        {
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПРОИЗВОДСТВЕННО АДМИНИСТРАТИВНАЯ ЗОНА") { Tag = StreetItemType.Noun, Tag2 = 1, Gender = Pullenti.Morph.MorphGender.Feminie };
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ЖИЛОЙ МАССИВ") { Tag = StreetItemType.Noun, Tag2 = 1, Gender = Pullenti.Morph.MorphGender.Masculine };
            t.AddAbridge("Ж.М.");
            t.AddAbridge("Ж/М");
            t.AddVariant("ЖИЛМАССИВ", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ЗОНА") { Tag = StreetItemType.Noun, Tag2 = 2, Gender = Pullenti.Morph.MorphGender.Feminie };
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("САД") { Tag = StreetItemType.Noun, Tag2 = 2, Gender = Pullenti.Morph.MorphGender.Masculine };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("УРОЧИЩЕ") { Tag = StreetItemType.Noun, Tag2 = 1, Gender = Pullenti.Morph.MorphGender.Neuter };
            t.AddAbridge("УР-ЩЕ");
            m_Ontology.Add(t);
            ...
            foreach (string s in new string[] {"МАРШАЛА", "ГЕНЕРАЛА", "АДМІРАЛА", "КОСМОНАВТА", "ЛЬОТЧИКА", "АВІАКОНСТРУКТОРА", "АРХІТЕКТОРА", "СКУЛЬПТОРА", "ХУДОЖНИКА", "КОНСТРУКТОРА", "АКАДЕМІКА", "ПРОФЕСОРА", "ЛЕЙТЕНАНТА", "КАПІТАН", "КАПІТАН-ЛЕЙТЕНАНТА", "МАЙОР", "ПІДПОЛКОВНИК", "ПОЛКОВНИК", "ПОЛІЦІЇ", "МІЛІЦІЇ"}) 
            {
                m_StdOntMisc.Add(new Pullenti.Ner.Core.Termin(s));
            ...
            t = new Pullenti.Ner.Core.Termin("ВАСИЛЬЕВСКОГО ОСТРОВА") { Tag = StreetItemType.StdName, Acronym = "ВО" };
            t.AddAbridge("В.О.");
            m_Ontology.Add(t);
        ...
        static string[] m_RegTails = new string[] {"ГОРОДОК", "РАЙОН", "МАССИВ", "МАСИВ", "КОМПЛЕКС", "ЗОНА", "КВАРТАЛ", "ОТДЕЛЕНИЕ", "ПАРК", "МЕСТНОСТЬ", "РАЗЪЕЗД", "УРОЧИЩЕ", "САД"};
        ...
        public static StreetItemToken TryParse(Pullenti.Ner.Token t, StreetItemToken prev = null, bool inSearch = false, Pullenti.Ner.Geo.Internal.GeoAnalyzerData ad = null)
        {
            ...
            if (res != null && res.Typ != StreetItemType.Noun) 
            {
                ...
                if (tt != null && tt.IsChar('(')) 
                {
                    ...
                    {
                        res.OrtoTerr = ait;
                        res.EndToken = ait.EndToken.Next;
                    }
                    else 
                    {
        ...
        public static StreetItemToken _tryParse(Pullenti.Ner.Token t, bool ignoreOnto, StreetItemToken prev, bool inSearch)
        {
            ...
            if (tn != null) 
            {
                ...
                if (org != null) 
                    return new StreetItemToken(t0, org.EndToken) { Typ = StreetItemType.Fix, Org = org };
            ...
            if ((((nt = t as Pullenti.Ner.NumberToken))) != null) 
            {
                ...
                {
                    if (prev != null && prev.Typ == StreetItemType.Noun && (((t.Next.IsHiphen || prev.Termin.CanonicText == "КВАРТАЛ" || prev.Termin.CanonicText == "ЛИНИЯ") || prev.Termin.CanonicText == "АЛЛЕЯ" || prev.Termin.CanonicText == "ДОРОГА"))) 
                    {
            ...
            if (tok != null && !ignoreOnto) 
            {
                ...
                switch ((StreetItemType)tok.Termin.Tag)
                {
                    ...
                    case StreetItemType.Noun:
                        ...
                        if ((!abr && tok.BeginToken == tok.EndToken && tok.BeginToken.Chars.IsCapitalUpper) && tok.BeginToken.GetMorphClassInDictionary().IsNoun) 
                        {
                    ...
                    case StreetItemType.StdName:
                        ...
                        if (tok.BeginToken.Chars.IsAllLower && !isPostOff && tok.EndToken.Chars.IsAllLower) 
                        {
                            ...
                            else if (prev != null && ((prev.Typ == StreetItemType.Number || prev.Typ == StreetItemType.Noun))) 
                            {
                            }
                            else 
                                return null;
                        ...
                        if (tok.Termin.AdditionalVars != null && tok.Termin.AdditionalVars.Count > 0) 
                        {
                            ...
                            if (tok.Termin.AdditionalVars.Count > 1) 
                            {
                                if (tok.Termin.AdditionalVars[0].CanonicText.IndexOf(' ') < 0) 
                                {
                                    sits.AltValue2 = sits.Value;
                                    sits.Value = tok.Termin.AdditionalVars[1].CanonicText;
                                }
                                else 
                                    sits.AltValue2 = tok.Termin.AdditionalVars[1].CanonicText;
                            }
                        }
                        if (tok.BeginToken != tok.EndToken && !isPostOff) 
                        {
                            ...
                            {
                                if (((m_StdOntMisc.TryParse(tok.BeginToken, Pullenti.Ner.Core.TerminParseAttr.No) != null || tok.BeginToken.GetMorphClassInDictionary().IsProperName || (tok.BeginToken.LengthChar < 4))) && tok.EndToken.LengthChar > 2 && ((tok.EndToken.Morph.Class.IsProperSurname || !tok.EndToken.GetMorphClassInDictionary().IsProperName))) 
                    ...
                    case StreetItemType.StdPartOfName:
                        ...
                        if (sit.Typ == StreetItemType.Noun) 
                        {
                            if (tok.Morph.Number == Pullenti.Morph.MorphNumber.Plural) 
                                ...
                                return new StreetItemToken(tok.BeginToken, tt1) { Typ = StreetItemType.Name, Morph = tok.Morph, Termin = tok.Termin };
                        }
                        if (sit.Value != null) 
                        {
                            if (sit.AltValue == null) 
                                sit.AltValue = string.Format("{0} {1}", vvv, sit.Value);
                            else 
                                sit.Value = string.Format("{0} {1}", vvv, sit.Value);
                        }
                        else if (sit.ExistStreet == null) 
                        {
                            sit.AltValue = (sit.BeginToken as Pullenti.Ner.TextToken).Term;
                            sit.Value = string.Format("{0} {1}", vvv, (sit.BeginToken as Pullenti.Ner.TextToken).Term);
                        }
            ...
            if (tt != null) 
            {
                ...
                {
                    Pullenti.Ner.Geo.Internal.NameToken nam = Pullenti.Ner.Geo.Internal.NameToken.TryParse(tt.Next, Pullenti.Ner.Geo.Internal.NameTokenType.City, 0, false);
                ...
                if (tt.Chars.IsCyrillicLetter && tt.LengthChar > 1 && !tt.Morph.Class.IsPreposition) 
                {
                    ...
                    else if (prev != null && prev.Typ == StreetItemType.Noun) 
                    {
                        ...
                        if (!ok1) 
                        {
                            ...
                            if (tt1 != null && (tt1.GetReferent() is Pullenti.Ner.Geo.GeoReferent)) 
                                ok1 = true;
                            else if (Pullenti.Ner.Geo.Internal.MiscLocationHelper.IsUserParamAddress(prev)) 
                                ok1 = true;
                            else 
                            {
                                Pullenti.Ner.Geo.Internal.GeoAnalyzerData ad = Pullenti.Ner.Geo.GeoAnalyzer.GetData(t);
                    ...
                    if (ok1) 
                    {
                        ...
                        else if ((tt.WhitespacesAfterCount < 2) && (tt.Next is Pullenti.Ner.TextToken) && tt.Next.Chars.IsLetter) 
                        {
                            ...
                            else if (!AddressItemToken.CheckHouseAfter(tt.Next, false, false) || tt.Next.IsNewlineAfter) 
                            {
                                ...
                                else if (nn.Typ == StreetItemType.Noun) 
                                {
                                    ...
                                    {
                                        res.Value = Pullenti.Ner.Core.MiscHelper.GetTextValue(res.BeginToken, res.EndToken, Pullenti.Ner.Core.GetTextAttr.No);
                                        string var = Pullenti.Morph.MorphologyService.GetWordform(res.Value, new Pullenti.Morph.MorphBaseInfo() { Case = Pullenti.Morph.MorphCase.Nominative, Class = Pullenti.Morph.MorphClass.Adjective, Number = Pullenti.Morph.MorphNumber.Singular, Gender = gen });
                        ...
                        if (res != null && res.Typ == StreetItemType.Name && (res.WhitespacesAfterCount < 2)) 
                        {
                            ...
                            if ((tt != null && tt.LengthChar == 1 && tt.Chars.IsAllUpper) && tt.Next != null && tt.Next.IsChar('.')) 
                            {
        ...
        internal static List<StreetItemToken> TryParseSpec(Pullenti.Ner.Token t, StreetItemToken prev)
        {
            ...
            if (prev != null && prev.Typ == StreetItemType.Noun) 
            {
                ...
                if ((num != null && num.IntValue != null && !t.IsValue("С", null)) && !t.IsValue("C", null)) 
                {
                    res = new List<StreetItemToken>();
                    res.Add((sit = new StreetItemToken(num.BeginToken, num.EndToken) { Typ = StreetItemType.Number, Number = num }));
                    t = num.EndToken.Next;
                    if ((num.Typ == Pullenti.Ner.NumberSpellingType.Digit && (t is Pullenti.Ner.TextToken) && t.Chars.IsLetter) && !t.IsWhitespaceBefore && t.LengthChar == 1) 
                    {
                        sit.EndToken = t;
