...
namespace Pullenti.Ner.Org
{
    ...
    public class OrganizationAnalyzer : Pullenti.Ner.Analyzer
    {
        ...
        static List<Pullenti.Ner.ReferentToken> TryAttachOrgs(Pullenti.Ner.Token t, int step)
        {
            ...
            {
                string s = tt.Term;
                if (((s.StartsWith("ГУ") || s.StartsWith("РУ") || s.StartsWith("МУ"))) && s.Length > 3 && ((s.Length > 4 || s == "ГУВД"))) 
                {
                    ...
                    if (rt != null) 
                    {
                        ...
                        typ.Typ = (s.StartsWith("ГУ") ? "главное управление" : (s.StartsWith("МУ") ? "межмуниципальное" : "региональное управление"));
