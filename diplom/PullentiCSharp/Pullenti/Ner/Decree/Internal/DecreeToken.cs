...
namespace Pullenti.Ner.Decree.Internal
{
    ...
    public class DecreeToken : Pullenti.Ner.MetaToken
    {
        ...
        static Pullenti.Ner.Token _tryAttachNumber(Pullenti.Ner.Token t, StringBuilder tmp, bool afterNum)
        {
            ...
            for (; t2 != null; t2 = t2.Next) 
            {
                ...
                if (t2.IsTableControlChar) 
                    break;
                if (t2.IsCharOf(".:") && t2.IsWhitespaceAfter) 
                    break;
            ...
            {
                if (t1.IsValue("РД", null) || t1.IsValue("ПД", null) || t1.IsValue("ПП", null)) 
                {
        ...
        public static DecreeToken TryAttachNameToEndOfLine(Pullenti.Ner.Token t)
        {
            if (t == null || ((t.IsNewlineBefore && t.Previous != null))) 
                return null;
            ...
            for (Pullenti.Ner.Token tt = t; tt != null && (cou < 100); tt = tt.Next,cou++) 
            {
                ...
                PartToken v = PartToken.TryAttach(tt, null, false, false);
                if (v != null) 
                    return null;
                t1 = tt;
            }
        ...
        public static DecreeToken TryAttachName(Pullenti.Ner.Token t, string typ, bool veryProbable = false, bool inTitleDocRef = false)
        {
            ...
            else 
            {
                ...
                if (typ == "РУКОВОДЯЩИЙ ДОКУМЕНТ" && t.Chars.IsLetter && !t.Chars.IsAllLower) 
                    abou = true;
                else if (ty != Pullenti.Ner.Decree.DecreeKind.Konvention && ty != Pullenti.Ner.Decree.DecreeKind.Program && ty != Pullenti.Ner.Decree.DecreeKind.Ustav) 
                    return null;
            }
            if (t == null) 
                return null;
