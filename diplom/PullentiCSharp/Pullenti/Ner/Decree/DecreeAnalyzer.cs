    ...
    public class DecreeAnalyzer : Pullenti.Ner.Analyzer
    {
        ...
        static List<Pullenti.Ner.ReferentToken> _TryAttach(List<Pullenti.Ner.Decree.Internal.DecreeToken> dts, Pullenti.Ner.Decree.Internal.DecreeToken baseTyp, bool afterDecree, Pullenti.Ner.Core.AnalyzerData ad)
        {
            ...
            if (dts.Count == 2 && dts[0].Typ == Pullenti.Ner.Decree.Internal.DecreeToken.ItemType.Org && dts[1].Typ == Pullenti.Ner.Decree.Internal.DecreeToken.ItemType.Typ) 
                return null;
            if ((dts.Count == 2 && dts[0].Typ == Pullenti.Ner.Decree.Internal.DecreeToken.ItemType.Typ && (dts[0].LengthChar < 3)) && dts[1].Typ == Pullenti.Ner.Decree.Internal.DecreeToken.ItemType.Number) 
            {
                if (!dts[0].IsWhitespaceAfter && dts[0].IsDoubtful) 
                    return null;
            }
            if (dts.Count == 1) 
            {
            ...
            if (dec.Typ == "РУКОВОДЯЩИЙ ДОКУМЕНТ" && !et.IsNewlineAfter) 
            {
                ...
                if (typ1 != null && typ1.Typ == Pullenti.Ner.Decree.Internal.DecreeToken.ItemType.Typ && typ1.Value == dec.Typ) 
                {
                    ...
                    Pullenti.Ner.Decree.Internal.DecreeToken dn = Pullenti.Ner.Decree.Internal.DecreeToken.TryAttachName(tt, dec.Typ, false, false);
                    if (dn != null) 
                    {
            ...
            if (dec.FindSlot(DecreeReferent.ATTR_NAME, null, true) == null && !rt.IsNewlineBefore) 
            {
                ...
                Pullenti.Ner.Decree.Internal.DecreeToken nam = Pullenti.Ner.Decree.Internal.DecreeToken.TryAttachName(tt0, dec.Typ, false, false);
            ...
            if (dec.FindSlot(DecreeReferent.ATTR_NAME, null, true) == null && !rt.IsNewlineAfter) 
            {
                if (rt.IsNewlineBefore || ((rt.BeginToken.Previous != null && rt.BeginToken.Previous.LengthChar == 1 && ((rt.BeginToken.Previous.IsNewlineBefore || rt.BeginToken.Previous.IsTableControlChar))))) 
                {
                    ...
                    {
                        Pullenti.Ner.Decree.Internal.DecreeToken nam = Pullenti.Ner.Decree.Internal.DecreeToken.TryAttachNameToEndOfLine(rt.EndToken.Next);
                        if (nam != null) 
                        {
            ...
            Pullenti.Ner.ReferentToken rt22 = DecreeAnalyzer._tryAttachApproved(rt.EndToken.Next, ad, true, false);
            if (rt22 != null) 
            {
        ...
        public override void Process(Pullenti.Ner.Core.AnalysisKit kit)
        {
            ...
            for (Pullenti.Ner.Token t = kit.FirstToken; t != null; t = t.Next,lastDecDist++) 
            {
                ...
                if (((dts != null && dts.Count > 1 && tok != null) && dts[0].EndToken == tok.EndToken && dts[0].Typ == Pullenti.Ner.Decree.Internal.DecreeToken.ItemType.Typ) && dts[1].Typ != Pullenti.Ner.Decree.Internal.DecreeToken.ItemType.Terr) 
                    tok = null;
                if (tok != null) 
                {
        ...
        internal static Pullenti.Ner.ReferentToken TryAttachApproved(Pullenti.Ner.Token t, Pullenti.Ner.Core.AnalyzerData ad)
        {
            ...
            {
                Pullenti.Ner.ReferentToken rt0 = _tryAttachApproved(br.EndToken.Next, ad, false, true);
                if (rt0 != null) 
                {
            ...
            for (tt = tt.Next; tt != null; tt = tt.Next) 
            {
                ...
                if (Pullenti.Ner.Decree.Internal.DecreeToken.IsKeyword(tt, false) != null && tt.Chars.IsCapitalUpper) 
                    break;
                Pullenti.Ner.ReferentToken rt0 = _tryAttachApproved(tt, ad, true, false);
                if (rt0 != null) 
                {
                    ...
                    {
                        (rt0.Referent as DecreeReferent).AddTyp(dt);
                        if (dt.EndToken.Next != null && dt.EndToken.Next.IsValue("О", "ПРО")) 
                        {
                            t00 = dt.EndToken.Next;
                            nam = Pullenti.Ner.Core.MiscHelper.GetTextValue(t00, t1, Pullenti.Ner.Core.GetTextAttr.KeepRegister);
                        }
                    }
        ...
        internal static Pullenti.Ner.ReferentToken _tryAttachApproved(Pullenti.Ner.Token t, Pullenti.Ner.Core.AnalyzerData ad, bool mustBeComma = true, bool nameInBrackets = false)
        {
        ...
        internal static List<Pullenti.Ner.MetaToken> TryAttachParts(List<Pullenti.Ner.Decree.Internal.PartToken> parts, Pullenti.Ner.Decree.Internal.DecreeToken baseTyp, Pullenti.Ner.Referent _defOwner)
        {
            ...
            if (parts[parts.Count - 1].Typ != Pullenti.Ner.Decree.Internal.PartToken.ItemType.Subprogram && parts[parts.Count - 1].Typ != Pullenti.Ner.Decree.Internal.PartToken.ItemType.AddAgree) 
            {
                ...
                if (thisDec != null) 
                {
                    if ((_defOwner is DecreeReferent) && (((_defOwner as DecreeReferent).Typ0 == thisDec.Typ || (((_defOwner as DecreeReferent).Kind == DecreeKind.Kodex && thisDec.Typ == "КОДЕКС")) || parts[0].Typ == Pullenti.Ner.Decree.Internal.PartToken.ItemType.Appendix))) 
                    {
                    }
                    else 
                        _defOwner = null;
